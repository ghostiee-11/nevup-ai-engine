...
The repo ships a [`render.yaml`](./render.yaml) Blueprint that provisions both the API container and a Postgres-with-pgvector database in one step.

### One-time setup

```mermaid
flowchart LR
    A[Push repo to GitHub<br/>public] --> B[Render: New +<br/>→ Blueprint]
    B --> C[Connect repo<br/>Render reads render.yaml]
    C --> D[Set 3 env vars in dashboard:<br/>JWT_SECRET, GEMINI_API_KEY,<br/>GROQ_API_KEY]
    D --> E[Deploy starts:<br/>build → migrate → seed → uvicorn]
    E --> F[Copy live URL → GitHub repo<br/>variable RENDER_API_URL]
    F --> G[Keep-alive cron pings /health<br/>every 10 min]
...
2. On Render: **`New +`** → **`Blueprint`** → connect the GitHub repo. Render reads `render.yaml` and proposes the two services (`nevup-api` web service + `nevup-db` Postgres).
3. **Set the three secret env vars** in the Render dashboard for `nevup-api` (these have `sync: false` in the blueprint so they're not committed):

   | Key | Value |
   |---|---|
   | `JWT_SECRET` | the shared HS256 secret |
   | `GEMINI_API_KEY` | your Gemini key |
   | `GROQ_API_KEY` | your Groq key |

4. Click **`Apply`**. Render builds the Docker image, runs `entrypoint.sh` (waits for DB → `alembic upgrade head` → `python -m scripts.seed` → `uvicorn`), and exposes the URL — typically `https://nevup-api.onrender.com`.

### Keep-alive cron (prevents the 15-min spin-down)

Render's free Web Service plan spins the container down after 15 minutes of inactivity (~30s cold start). The repo includes [`.github/workflows/keep-alive.yml`](./.github/workflows/keep-alive.yml) which pings `/health` every 10 minutes via GitHub Actions cron.

**Activate it:**

1. In the GitHub repo: **Settings** → **Secrets and variables** → **Actions** → **Variables** tab → **`New repository variable`**
2. Name: `RENDER_API_URL` · Value: your Render URL (no trailing slash)

Without this variable set, the workflow no-ops gracefully — no failure spam.

### Render free tier reality check

| Constraint | Workaround |
|---|---|
| Web Service spins down after 15 min idle | GitHub Actions keep-alive (built in) |
| Free Postgres expires after **90 days**, then deleted | Migrate to a paid plan or self-hosted before day 90 |
| Free instance: 512 MB RAM, 0.1 CPU | Plenty for this workload (single uvicorn worker) |
| Region: `oregon` only on free tier | Higher latency for non-US clients (mitigate with persistent SSE connections) |
| First boot may take 2-3 min | Subsequent deploys are ~45s |
